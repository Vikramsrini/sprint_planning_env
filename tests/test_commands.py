...
class TestCommandParser:
    ...
    def board(self):
        b = ProjectBoard()
        b.reset({"sprint_stories": ["US-101", "US-103", "US-105"]})
        return b
    ...
    def test_list_backlog(self, board):
        ...
        assert "US-101" in output

    def test_view_story(self, board):
        output, error = parse_and_execute("VIEW_STORY US-101", board)
        assert error is None
        assert "User Authentication" in output

    def test_view_story_not_found(self, board):
        output, error = parse_and_execute("VIEW_STORY US-999", board)
    ...
    def test_check_deps(self, board):
        output, error = parse_and_execute("CHECK_DEPS US-105", board)
        assert error is None
        assert "US-101" in output  # US-105 depends on US-101
    ...
    def test_estimate(self, board):
        output, error = parse_and_execute("ESTIMATE US-101 8", board)
        assert error is None
        assert "estimated" in output.lower()

    def test_estimate_invalid_points(self, board):
        output, error = parse_and_execute("ESTIMATE US-101 7", board)
        # 7 is not Fibonacci
        assert "Fibonacci" in output or "Invalid" in output

    def test_assign(self, board):
        output, error = parse_and_execute("ASSIGN US-101 Alice", board)
        assert error is None
        assert "assigned" in output.lower()

    def test_assign_unknown_dev(self, board):
        output, error = parse_and_execute("ASSIGN US-101 Zach", board)
        assert "not found" in output.lower()

    def test_add_to_sprint(self, board):
        output, error = parse_and_execute("ADD_TO_SPRINT US-102", board)
        assert error is None
        assert "added" in output.lower()

    def test_remove_from_sprint(self, board):
        output, error = parse_and_execute("REMOVE_FROM_SPRINT US-101", board)
        assert error is None
        assert "removed" in output.lower()

    def test_flag_risk(self, board):
        output, error = parse_and_execute("FLAG_RISK US-101 vague acceptance criteria", board)
        assert error is None
...
class TestDestructiveDetection:
    ...
    def test_delete_story(self):
        assert is_destructive("DELETE_STORY US-101") is True
    ...
    def test_assign_not_destructive(self):
        assert is_destructive("ASSIGN US-101 Alice") is False
...
class TestCommandTypes:
    ...
    def test_investigation(self):
        ...
        assert get_command_type("VIEW_STORY US-101") == "investigation"
        assert get_command_type("CHECK_DEPS US-105") == "investigation"
        assert get_command_type("VIEW_TEAM") == "investigation"

    def test_planning(self):
        assert get_command_type("ESTIMATE US-101 5") == "planning"
        assert get_command_type("ASSIGN US-101 Alice") == "planning"
        assert get_command_type("ADD_TO_SPRINT US-102") == "planning"

    def test_destructive(self):
        assert get_command_type("DELETE_STORY US-101") == "destructive"
...
class TestBoardState:
    ...
    def test_estimate_updates_state(self):
        ...
        assert board.estimates.get("US-103") is None
        board.estimate("US-103", 5)
        assert board.estimates.get("US-103") == 5

    def test_assign_updates_state(self):
        board = ProjectBoard()
        board.reset({"sprint_stories": ["US-101"]})
        board.assign("US-101", "Alice")
        assert board.assignments.get("US-101") == "Alice"

    def test_unassign(self):
        board = ProjectBoard()
        board.reset({"sprint_stories": ["US-101"], "initial_assignments": {"US-101": "Alice"}})
        board.unassign("US-101")
        assert "US-101" not in board.assignments

    def test_add_remove_sprint(self):
        board = ProjectBoard()
        board.reset({"sprint_stories": ["US-101"]})
        assert "US-101" in board.sprint_stories
        board.remove_from_sprint("US-101")
        assert "US-101" not in board.sprint_stories
        board.add_to_sprint("US-101")
        assert "US-101" in board.sprint_stories

    def test_flag_risk(self):
        board = ProjectBoard()
        board.reset({"sprint_stories": ["US-114"]})
        board.flag_risk("US-114", "vague acceptance criteria")
        assert "US-114" in board.risk_flags

    def test_metrics(self):
        board = ProjectBoard()
        board.reset({
            "sprint_stories": ["US-101", "US-103"],
            "initial_assignments": {"US-101": "Alice"},
        })
        metrics = board.get_metrics()
        assert metrics["sprint_stories"] == 2
        assert metrics["unassigned_count"] == 1  # US-103 unassigned

    def test_circular_dependency_detection(self):
        board = ProjectBoard()
        board.reset({
            "sprint_stories": ["US-105", "US-113"],
            "circular_deps": {"US-105": "US-113", "US-113": "US-105"},
        })
        output = board.check_deps("US-105")
